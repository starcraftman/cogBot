...
from cogdb.eddb import (Conflict, ConflictState, Faction, Influence, System, Station,
                        StationFeatures, FactionActiveState, FactionPendingState,
...
class EDMCJournal():
    ...
    def parse_system(self):
        ...
        try:
            ...
            if 'StarSystem' not in body:
                raise ValueError("No StarSystem found or message malformed.")
        except KeyError as e:
            raise ValueError("No StarSystem found or message malformed.") from e
        ...
        if "SystemEconomy" in body or "SystemSecondEconomy" in body:
            system['economies'] = []
        ...
        try:
            ...
            raise ValueError("System not found: [%s]" % system['name'])
    ...
    def parse_station(self):
        ...
        Raises:
            ValueError: One or more prerequisites were not met to continue parsing.
        """
        try:
            body = self.body
            system = self.parsed['system']
            if 'StationName' not in body or 'id' not in system:
                raise ValueError("No Station or system not parsed before.")
        except KeyError as e:
            raise ValueError("No Station or system not parsed before.") from e
        ...
        if "StationServices" in body:
            to_look_for = [x for x in StationFeatures.__dict__.keys() if x not in ('id', 'station') and not x.startswith('_')]
            station['features'] = {x: True if x in body["StationServices"] else False for x in to_look_for}
        ...
        try:
            ...
            if station['features']:
                station['features']['id'] = station_db.id
            self.db_objs['station'] = station_db
        except sqla_orm.exc.NoResultFound:
            raise ValueError("Station not found in system: [%s] in %s" % (system['name'], station['name']))
    ...
    def parse_factions(self):
        ...
        factions, p_faction = {}, {}
        for faction in self.body['Factions']:
            for key in ('Name', 'Influence'):
                if key in faction:
                    p_faction[key.lower()] = faction[key]

            for key in ("Allegiance", "Government"):
                if key in faction:
                    p_faction["{}_id".format(key.lower())] = MAPS[key][faction[key]]
            if "Happiness" in faction and faction["Happiness"]:
                p_faction['happiness_id'] = int(faction["Happiness"][-2])

            factions[p_faction['name']] = p_faction
            p_faction = {}

        faction_dbs = {x.name: x for x in self.session.query(Faction).filter(Faction.name.in_(factions.keys())).all()}
        for faction in self.body['Factions']:
            p_faction = factions[faction['Name']]
            p_faction['id'] = faction_dbs[faction['Name']].id

            for key, cls in [("ActiveStates", FactionActiveState),
                             ("PendingStates", FactionPendingState),
                             ("RecoveringStates", FactionRecoveringState)]:
                if key in faction:
                    p_faction[camel_to_c(key)] = [{
                        'system_id': system['id'],
                        'faction_id': p_faction['id'],
                        'state_id': MAPS['FactionState'][x['State']]
                    }
                        for x in faction[key]
                    ]
    ...
    def parse_conflicts(self):
        try:
            ...
            if 'Conflicts' not in self.body or 'id' not in system:
                raise ValueError("No Conflicts or system not parsed before.")
        except KeyError as e:
            raise ValueError("No Conflicts or system not parsed before.") from e
        ...
        self.db_objs['conflicts'] = [Conflict(**x) for x in conflicts]
