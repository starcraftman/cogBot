...
def test_base_loads(base_json):
    expect_control = spy.SpySystem(
        ed_system_id=10477373803, power_id=9, power_state_id=16, is_expansion=False,
        income=122, upkeep_current=0, upkeep_default=21, fort_trigger=5211, um_trigger=33998
    )
    expect_taking = spy.SpySystem(
        ed_system_id=11665533904241, power_id=9, power_state_id=64, is_expansion=True,
...
def test_refined_loads(base_json, refined_json):
    expect_prep = spy.SpyPrep(power_id=9, ed_system_id=2557887812314, merits=14140)
    expect_vote = spy.SpyVote(power_id=11, vote=78)
    expect_expo = spy.SpySystem(power_id=6, ed_system_id=2106438158699, fort=1247, um=53820, is_expansion=True)
    expect_sys = spy.SpySystem(power_id=11, ed_system_id=22958210698120, fort=464, um=900, is_expansion=False)

    systems = spy.load_base_json(base_json)
    preps, votes, systems = spy.load_refined_json(refined_json, systems)
