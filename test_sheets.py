...
def fort_sheet():
    ...
    f_sheet = sheets.SheetApi(sheet_id, secrets['json'], secrets['token'])
...
def fort_sheet_reset():
    ...
    # Ensure scratch cells always reset, stuck in catch22 batch_set must work
    cell_ranges = ['!B16:B17', '!F6:G6']
    n_vals = [[['Shepron'], ['Grimbald']], [[4910, 4350]]]
    f_sheet.batch_set(cell_ranges, n_vals)
...
def test_set(fort_sheet_reset):
    fort_sheet_reset.set('!B16:B16', [['NotShepron']])
    assert fort_sheet_reset.get('!B16:B16') == [['NotShepron']]

def test_batch_set(fort_sheet_reset):
    cell_ranges = ['!B16:B17', '!F6:G6']
    n_vals = [[['NotShepron'], ['Grimbald']], [[2222, 3333]]]
    fort_sheet_reset.batch_set(cell_ranges, n_vals)
